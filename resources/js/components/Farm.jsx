...
function Farm() {
  ...
  const {
    farm, show: farmsShow, loading: farmsLoading, error: farmsError,
  } = useFarmService();
  const {
    turbines, index: turbinesIndex, loading: turbinesLoading, error: turbinesError,
  } = useTurbineService();
  const {
  ...
  const mergeData = () => turbines.map((turbine) => {
