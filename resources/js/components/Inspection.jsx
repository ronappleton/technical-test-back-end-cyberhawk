...
function Inspection() {
  ...
  const {
    inspection, show: inspectionsShow, loading: inspectionsLoading, error: inspectionsError,
  } = useInspectionService();
  const {
    turbine, show: turbinesShow, loading: turbinesLoading, error: turbinesError,
  } = useTurbineService();
  const {
    components, index: componentsIndex, loading: componentsLoading, error: componentsError,
  } = useComponentService();
  const {
    grades, index: gradesIndex, loading: gradesLoading, error: gradesError,
  } = useGradeService();
  const {
    componentTypes, index: componentTypesIndex, loading: componentTypesLoading, error: componentTypesError,
  } = useComponentTypeService();
  const {
    gradeTypes, index: gradeTypesIndex, loading: gradeTypesLoading, error: gradeTypesError,
  } = useGradeTypeService();
  ...
  const mergeData = () => components.map((component) => {
    if (grades.length === 0) {
    ...
    if (componentTypes.length !== 0) {
      const componentType = componentTypes.find((type) => type.id === component.component_type_id);
      if (componentType) {
